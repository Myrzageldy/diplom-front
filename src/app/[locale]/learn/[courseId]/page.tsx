...
export default function LearnPage() {
  ...
  const handleGetCertificate = async () => {
    ...
    try {
      ...
      setCertError(e.detail || 'Не удалось получить сертификат');
    } finally {
      setCertLoading(false);
    }
  };

  const handleDownloadCertificate = async (certNumber: string) => {
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('access_token') : null;
      const res = await fetch(
        `http://localhost:8000/api/courses/certificates/${certNumber}/download/`,
          ...
          <div className="w-80 h-full flex flex-col">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
              ...
              {progress.progress_percent === 100 && (
                <div className="mt-4 p-3 bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl">
                  ...
                  {certificate ? (
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 w-full py-2 bg-amber-500 hover:bg-amber-400 text-white rounded-lg text-xs font-semibold transition-colors"
                      >
                    ...
                    <>
                      {certError && (
                        <p className="text-xs text-rose-600 dark:text-rose-400 mb-2">{certError}</p>
                      )}
                      <button
                        onClick={handleGetCertificate}
                        disabled={certLoading}
                        className="w-full py-2 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-white rounded-lg text-xs font-semibold transition-all shadow-sm shadow-amber-500/25 disabled:opacity-60 flex items-center justify-center gap-1.5"
                      >
                        {certLoading ? (
